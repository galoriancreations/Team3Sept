<template>
    <div class="v-radar">
      <svg width="750px" :height="svgSize" :viewBox="svgViewBox">
        <polygon
                :points="buildStructure(1.1)"
                :fill="transparent"
                :stroke="externalStrokeColor"
                :stroke-width="externalStrokeWidth"
        ></polygon>
        <polygon>
                :points="buildStructure(n/4.5)"
                :fill="transparent"
  ...
  <script>
      export default {
          ...
          methods: {
              ...
              xLineCoordinate(loopIndex) {
                  let newAngle = calculateNewAngle((loopIndex - 1), this.baseAngle);
  
                  return calculateXCoordinate(newAngle, this.radius, this.originPoint);
              },
              yLineCoordinate(loopIndex) {
                  let newAngle = calculateNewAngle((loopIndex - 1), this.baseAngle);
  
                  return calculateYCoordinate(newAngle, this.radius, this.originPoint);
              },
              xTextCoordinate(loopIndex) {
                  let newAngle = calculateNewAngle((loopIndex - 1), this.baseAngle);
  
                  return calculateXCoordinate(newAngle, parseFloat(this.radius) +200, this.originPoint) -100;
              },
              yTextCoordinate(loopIndex) {
                  let newAngle = calculateNewAngle((loopIndex - 1), this.baseAngle);
  
                  return calculateYCoordinate(newAngle, parseFloat(this.radius) + 100, this.originPoint) + 10;
              },
          },
          computed: {
              //test
